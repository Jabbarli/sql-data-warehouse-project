...
Script purpose:
	...
	- Truncates the silver layer tables before loading the data.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			cst_id,
			cst_key,
			TRIM(cst_firstname) AS cst_firstname, -- Trim the columns that contain unwanted spaces
		...
		FROM (
			SELECT 
				*, 
				ROW_NUMBER() OVER(PARTITION BY cst_id ORDER BY cst_create_date DESC) AS flag_last -- Select most recent record per customer
		...
		SELECT 
			...
			CAST(
				...
				AS DATE
				) AS prd_end_dt  -- Calculate end date as one day before the next start date for each product
